		...
		$.ajax({
			...
			success:function(respuesta){
				if(respuesta.existeUsuario){
					...
					obtenerFollowing(respuesta.codigoUsuario);
					obtenerFollowers(respuesta.codigoUsuario);
				}
				else
...
function obtenerFollowing(id){
	$.ajax({
		url:'/obtenerFollowing',
		method:'POST',
		dataType:'json',
		data:"codigoUsuario="+id,
		success:function(respuesta){
			$("#div-following").html('');
			// cargamos los following
			for(var i =0; i<respuesta.length;i++){
				$("#div-following").append(
					...
					'</div>'+
					'<div class="col-lg-9 col-sm-9 col-9 col-md-9">'+
					'  <b>'+respuesta[i].nombre+'</b>'+
					'  <p>'+respuesta[i].lugar+'</p>'+
					'</div>'
				);
			}
...
function obtenerFollowers(id){
	$.ajax({
		url:'/obtenerFollowers',
		data:'codigoUsuario='+id,
		dataType:'json',
		method:'POST',
		success:function(respuesta){
