...
app.get("/verificarUsuario/:nombreUsuario/:contrasenia/",function(request, response){
    ...
    const sqlUsuario = 'select count(1) cantidadUsuarios '+
					  'from tbl_usuarios '+
					  'where nickname = ? and contrasena = sha1(?);';
...
app.post("/obtenerFollowing",function(peticion, respuesta){
	
	const sqlFollowing = 'SELECT T.codigo_usuario_sigue, U.nombre, U.url_imagen_perfil, U.lugar '+
									'FROM tbl_usuarios U '+
									'INNER JOIN '+
									'(select codigo_usuario_sigue '+
									'from tbl_seguidores '+
									'where codigo_usuario = ?) T '+
									'on T.codigo_usuario_sigue = U.codigo_usuario';

	mysqlConexion.query(sqlFollowing, peticion.body.codigoUsuario,
...
app.post("/obtenerFollowers", function(peticion, respuesta){
	const sqlFollowers = 'SELECT U.nombre, U.url_imagen_perfil, U.lugar '+
						 'FROM tbl_usuarios U INNER JOIN '+
						 '(SELECT codigo_usuario '+
						 'FROM tbl_seguidores '+
						 'WHERE codigo_usuario_sigue = ?) S '+
						 'ON S.codigo_usuario = U.codigo_usuario';
	
	mysqlConexion.query(sqlFollowers, peticion.body.codigoUsuario,
		function(error, info, campos){
			respuesta.send(info);
		}
	);
